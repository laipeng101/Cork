...
public extension CachedDownloadsTracker
{
    func assignPackageTypeToCachedDownloads(brewPackagesTracker: BrewPackagesTracker)
    {
        ...
        for cachedDownload in cachedDownloads
        {
            ...
            if brewPackagesTracker.successfullyLoadedFormulae.contains(where: { $0.name(withPrecision: .general).localizedCaseInsensitiveContains(normalizedCachedPackageName) })
            ...
            else if brewPackagesTracker.successfullyLoadedCasks.contains(where: { $0.name(withPrecision: .general).localizedCaseInsensitiveContains(normalizedCachedPackageName) })
